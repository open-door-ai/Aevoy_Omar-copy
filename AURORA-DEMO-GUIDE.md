...
> How to demonstrate Aurora's always-listening AI wearable experience.

---

## Quick Start

1. Navigate to `https://www.aevoy.com/aurora` (or `localhost:3000/aurora` in dev)
...
If running the demo with the seeded test user, Aurora already knows:
...
To seed this data, run: `packages/agent/scripts/seed-aurora-test-user.sql` against Supabase.
...
### Scenario 3: "The Sarcasm Test" (Shows: Aurora knows when NOT to act)
...
3. **Intent detection** → feed card appears instantly when Aurora detects something actionable
4. **Contextual awareness** → Aurora uses stored knowledge about the user
5. **Appropriate non-action** → Aurora doesn't act on sarcasm, joint decisions, or vague mentions
...
### Cannot Do Yet
- **Ambient vs. directed speech**: Aurora can't distinguish "hey Aurora, do X" from "I was telling John about X". All speech from the mic is treated as ambient
...
- **Supabase**: Must be running with Aurora schema (migration 20260323_001)
