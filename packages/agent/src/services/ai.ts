...
async function trackApiCall(
  ...
  try {
    ...
    // Track usage stats (monthly counters)
    ...
    // NOTE: Credit wallet deduction is done ONCE per task at completion
    // (in processTaskV3 / processTask) using the task's total cost_usd.
    // Per-call deduction caused a $25+ gap because 78% of calls are sub-cent
    // and rounded to 0 — losing $9+ in real costs per week.
  } catch {
  ...
  if (userId) {
    ...
    if (orSettings?.enabled && orSettings.apiKey) {
      ...
      const orConfig: ModelConfig = {
        ...
        costPerMOutput: 3.00, // but Anticipy still tracks for platform fee
  ...
  for (const config of chain) {
    ...
    try {
      ...
      if (parsedActions.length === 0 && taskType !== 'generate' && taskType !== 'validate' && taskType !== 'respond') {
        ...
        if (emailMatch) syntheticActions.push({ type: 'send_email', params: { to: emailMatch[1], subject: taskSubject?.substring(0, 80) || 'Message from Anticipy', body: text.substring(0, 500) } } as Action);
...
export async function generateForcedDirectAnswer(
  ...
  const systemPrompt = `You are Anticipy, a done-state AI reporter. You have ALREADY run browser and search actions. Your ONLY job is to share results.
