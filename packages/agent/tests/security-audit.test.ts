...
describe('Encryption Security', () => {
  it('should use AES-256-GCM format (iv:authTag:data)', () => {
    ...
    const encryptionPath = '/workspaces/Aevoy_Omar-copy/packages/agent/src/security/encryption.ts';
    const fs = require('fs');
  ...
  it('should validate encryption key strength on startup', () => {
    const indexPath = '/workspaces/Aevoy_Omar-copy/packages/agent/src/index.ts';
    const fs = require('fs');
...
describe('Row-Level Security (RLS)', () => {
  it('should have RLS enabled on all user tables', () => {
    const migrationPath = '/workspaces/Aevoy_Omar-copy/apps/web/supabase/RUN_ALL_MIGRATIONS.sql';
    const fs = require('fs');
...
describe('API Key Protection', () => {
  ...
  it('should mask sensitive data in logs', () => {
    const loggingPath = '/workspaces/Aevoy_Omar-copy/packages/agent/src/utils/logging.ts';
    const fs = require('fs');
...
describe('PIN Security', () => {
  it('should enforce PIN lockout after failed attempts', async () => {
    ...
    const migrationPath = '/workspaces/Aevoy_Omar-copy/apps/web/supabase/migration_v15.sql';
    const fs = require('fs');
    const content = fs.readFileSync(migrationPath, 'utf8');
...
describe('Session Security', () => {
  it('should have session expiration (7 days)', async () => {
    const migrationPath = '/workspaces/Aevoy_Omar-copy/apps/web/supabase/migration_v4.sql';
    const fs = require('fs');
  ...
  it('should clean up expired sessions', async () => {
    const schedulerPath = '/workspaces/Aevoy_Omar-copy/packages/agent/src/services/scheduler.ts';
    const fs = require('fs');
