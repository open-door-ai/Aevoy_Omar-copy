...
const FILL_METHODS = [
    ...
    // 12. Aria label
    {
        name: 'aria_label',
        fn: async (page, target) => {
            if (!target.label && !target.placeholder)
                return false;
    ...
    // 14. Nth input on page (guessed by label)
    {
        ...
        fn: async (page, target) => {
            ...
            let index = 0;
            if (label.includes('email') || label.includes('username') || label.includes('first'))
                index = 0;
            else if (label.includes('password') || label.includes('last'))
                index = 1;
            else if (label.includes('confirm') || label.includes('phone'))
                index = 2;
    ...
    // 15. Textarea fallback
    {
        name: 'textarea',
        fn: async (page, target) => {
            if (!target.selector && !target.label)
                return false;
            ...
            const success = await page.evaluate(({ label, val }) => {
                function findInShadowRoots(root) {
                    ...
                    for (const input of inputs) {
                        const el = input;
                        if (el.name?.toLowerCase().includes(label.toLowerCase()) ||
...
export async function executeFill(page, target) {
                ...
                if (target.selector) {
                    try {
                        ...
                        if (actualValue === target.value) {
                            console.log(`[FILL] Method ${i + 1}/${FILL_METHODS.length} (${method.name}) succeeded`);
                            return { success: true, method: method.name, methodIndex: i + 1 };
                        }
                    }
                    catch {
                        // Verification failed, but method may have worked
                        return { success: true, method: method.name, methodIndex: i + 1 };
                    }
                }
                else {
