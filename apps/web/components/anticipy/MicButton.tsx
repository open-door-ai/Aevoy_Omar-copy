...
interface MicButtonProps {
  ...
  onIntentDetected?: (action: string, needsConfirmation?: boolean, queueId?: string) => void;
...
export function MicButton({
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "authenticated") {
            ...
            // Anticipy detected something actionable — notify the feed
            console.log("[MicButton] Intent detected:", msg.action, "needsConfirmation:", msg.needsConfirmation);
            onIntentDetected?.(msg.action, msg.needsConfirmation, msg.queueId);
          } else if (msg.type === "action_completed") {
            // Anticipy finished acting on a detected intent
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {isListening && (
        <p className="text-xs text-[--anticipy-text-secondary]">Tap to stop</p>
      )}
    </div>
  );
}
