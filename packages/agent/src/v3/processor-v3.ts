...
export async function processTaskV3(task: TaskRequest): Promise<TaskResult> {
  ...
  try {
    ...
    console.log(`[V3] Task ${taskId.slice(0, 8)} classified as ${classification.tier}${classification.tool ? ` (${classification.tool})` : ''} — tools: ${tierToolNames.length}/${totalToolCount}`);
