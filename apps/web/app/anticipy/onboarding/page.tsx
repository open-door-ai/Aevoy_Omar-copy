...
/* ─────────────────────────── Component ─────────────────────────── */
export default function AnticipyOnboarding() {
  ...
  const [messages, setMessages] = useState<Array<{ role: "user" | "anticipy"; text: string }>>([]);
  ...
  useEffect(() => {
    async function init() {
      ...
      if (profile?.onboarding_completed) {
        router.push("/anticipy");
        return;
      }

      // Show "Setting up Anticipy..." then transition to input
      setTimeout(() => {
        setPhase("ready");
      }, 2500);
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.response) {
          setMessages((prev) => [...prev, { role: "anticipy", text: data.response }]);
        }
      } else {
        setMessages((prev) => [
          ...prev,
          { role: "anticipy", text: "Something went wrong. Try again?" },
        ]);
      }
      ...
      if (user) {
        ...
        if (profile?.onboarding_completed) {
          setPhase("done");
          setTimeout(() => router.push("/anticipy"), 2000);
        }
      }
    } catch (err) {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "anticipy", text: "Couldn't connect. Try again." },
      ]);
    }
  ...
  if (phase === "loading") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-6">
        ...
        <p className="text-sm text-muted-foreground animate-pulse">Setting up Anticipy...</p>
      </div>
    );
  }
        ...
        {phase === "ready" && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center pt-20 gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-xs">
              Tell Anticipy about yourself. Name, what you do, how you like to be reached.
            </p>
          </div>
        )}
      ...
      <div className="sticky bottom-0 pt-4 bg-gradient-to-t from-background via-background to-transparent">
        <div className="relative flex items-center">
          <input
            ...
            placeholder="Tell Anticipy about yourself..."
