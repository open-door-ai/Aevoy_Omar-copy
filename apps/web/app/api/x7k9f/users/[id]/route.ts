...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // MUST include user_memory — monitoring jobs persist there and keep running
    const deletions = [
      ...
      supabase.from("user_memory").delete().eq("user_id", id),
      supabase.from("tasks").delete().eq("user_id", id),
      supabase.from("call_history").delete().eq("user_id", id),
      supabase.from("action_history").delete().eq("user_id", id),
    ];
