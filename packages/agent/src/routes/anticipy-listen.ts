/**
 * Anticipy Listen — Deepgram WebSocket Proxy (Hardened)
 *
 * Browser mic sends audio chunks to `/anticipy/listen/ws`.
 * This module proxies them to Deepgram for real-time transcription,
 * then feeds final transcripts into the context engine so Anticipy
...
export function setupListenWebSocket(server: Server): void {
  ...
  server.on('upgrade', (request: IncomingMessage, socket, head) => {
    ...
    if (url.pathname !== '/anticipy/listen/ws') {
