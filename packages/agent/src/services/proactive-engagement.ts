...
export class ProactiveEngagementEngine {
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          // Format and send via Anticipy Messenger (central delivery)
          const emailBody = this.formatDailyDigestEmail(digest);
          const result = await sendAnticipyMessage({
            userId: user.id,
            content: emailBody,
            priority: 'low',
            source: 'digest',
            emailSubject: `Your Daily Anticipy Digest - ${digest.date}`,
          });
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          const reportResult = await sendAnticipyMessage({
            userId: user.id,
            content: emailBody,
            priority: 'low',
            source: 'digest',
            emailSubject: `Your Weekly Anticipy Report - Productivity Score: ${report.productivity_score}/100`,
          });
  ...
  private formatDailyDigestEmail(digest: DailyDigest): string {
    return `
<h2>Your Daily Anticipy Digest</h2>
...
<p><em>This digest was automatically generated by your Anticipy AI. Reply to this email to interact with me!</em></p>
    `.trim();
  }
  ...
  private formatWeeklyReportEmail(report: WeeklyReport): string {
    return `
<h2>Your Weekly Anticipy Report</h2>
...
<p><em>This report was automatically generated by your Anticipy AI. Reply to this email to interact with me!</em></p>
    `.trim();
  }
  ...
  private async sendSuggestion(userId: string, suggestion: ProactiveSuggestion): Promise<void> {
    try {
      ...
      const result = await sendAnticipyMessage({
        ...
        source: 'proactive',
        emailSubject: `[Anticipy ${suggestion.type}] ${suggestion.message.substring(0, 50)}...`,
      });
