...
export default {
  async fetch(_request: Request, _env: Env): Promise<Response> {
    return new Response("Anticipy Email Router", { status: 200 });
  },
    ...
    try {
      ...
      if (CATCHALL_USERNAMES.includes(username.toLowerCase())) {
        ...
        if (!user) {
          ...
          message.setReject("No Anticipy account found for this email address. Sign up at aevoy.com");
          return;
        }
      } else {
      ...
      if (!isKnownSender && !isServiceEmail) {
        ...
        if (!user.unified_pin_hash) {
          ...
          await sendEmailViaAgent({
            ...
            html: `<p>This email address only accepts emails from ${username}'s registered email address.</p><p>To allow emails from other addresses, ask ${username} to set up a Security PIN in their Anticipy settings.</p>`,
