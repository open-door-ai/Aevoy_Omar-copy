 ...
 */
export async function executeLogin(page, params) {
    const methods = [
...
async function standardFormLogin(page, params) {
...
async function twoStepLogin(page, params) {
    await page.goto(params.url, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await page.waitForTimeout(1000);
...
async function enterKeyLogin(page, params) {
    await page.goto(params.url, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await page.waitForTimeout(1000);
    const emailField = page.locator('input[type="email"], input[name="email"], input[name="username"]');
    if ((await emailField.count()) === 0) {
...
async function tabNavigationLogin(page, params) {
    await page.goto(params.url, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await page.waitForTimeout(1000);
...
// Success Check
async function checkLoginSuccess(page, urlBefore) {
    await page.waitForTimeout(2000);
    const url = page.url();
