/**
 * Anticipy Messenger — Central delivery for ALL Anticipy outbound communication
 *
...
export interface AnticipyMessage {
  userId: string;
  content: string;
...
/**
 * Central delivery function for all Anticipy outbound communication.
 *
 ...
 */
async function selectChannel(message: AnticipyMessage): Promise<DeliveryChannel> {
...
async function deliverMessage(
  ...
  try {
    ...
    switch (channel) {
      ...
      case 'email': {
        const subject = emailSubject || '[Anticipy] Notification';
        return await sendResponse({
          to: profile.email,
          from: `${profile.username || 'anticipy'}@aevoy.com`,
          subject,
          body: content,
        });
      }
 ...
 */
async function queueForLater(message: AnticipyMessage): Promise<void> {
  try {
 ...
 */
async function scheduleEscalation(message: AnticipyMessage, originalChannel: DeliveryChannel): Promise<void> {
...
async function getUserProfile(userId: string): Promise<UserProfile | null> {
  ...
  try {
    ...
    const profile: UserProfile = {
      email: data.email,
      username: data.username || 'anticipy',
